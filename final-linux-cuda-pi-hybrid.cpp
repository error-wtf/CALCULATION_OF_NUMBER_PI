...
// ------------------------------
// Funktion: Berechnung eines Abschnitts von π mittels der BBP-Formel (CPU mit GMP)
// ------------------------------
...
mpf_class compute_chudnovsky_gpu(int terms_count, int threads_per_block) {
    ...
    // Reserviere Host-Arrays (für spätere Kopie)
    ...
    // Ergebnisse vom GPU-Gerät in Host-Arrays kopieren
    ...
    // Summiere die Terme: S = Σ (numerators[i] / denominators[i])
    mpf_set_default_prec(1000000);
    mpf_class S = 0;
    ...
    // Chudnovsky-Formel: π = (426880 * sqrt(10005)) / S
    mpf_class pi_chudnovsky = (426880 * sqrt(mpf_class(10005))) / S;
    return pi_chudnovsky;
}

// ------------------------------
// CPU-basierte multipräzise Berechnung der Chudnovsky-Methode mit GMP
// Diese Funktion liefert höhere Genauigkeiten, ist jedoch langsamer.
// ------------------------------
mpf_class compute_chudnovsky_cpu(int iterations) {
    mpf_set_default_prec(1000000);
    ...
    for (int k = 1; k < iterations; ++k) {
        ...
        M = (M * temp) / (k * k * k);
        // Umwandlung des konstanten Wertes als Stringliteral, um Mehrdeutigkeiten zu vermeiden
...
// Für niedrige Präzision (bis ca. 16 Nachkommastellen) wird die hybride Methode verwendet,
// ansonsten die CPU-basierte multipräzise Berechnung.
// ------------------------------
void calculate_pi_hybrid(int desiredDecimalPlaces, int terms_count, int threads_per_block, int bbp_terms) {
    if (desiredDecimalPlaces > 16) {
        ...
        std::cout << "\nNiedrige Präzision (bis ca. 16 Nachkommastellen) wird mittels hybrider Methode (GPU + CPU) berechnet." << std::endl;
...
int main() {
    ...
    // Heuristik: 2 MB pro 10 Nachkommastellen
    size_t max_digits_ram = (mainRamLimitMB / 2) * 10;
    
    fs::path currentPath = fs::current_path();
    auto diskSpace = fs::space(currentPath);
    size_t freeDiskBytes = diskSpace.free;
    size_t freeDiskMB = freeDiskBytes / (1024 * 1024);
    size_t max_digits_disk = (freeDiskMB / 2) * 10;
    
    size_t max_allowed_digits = std::min(max_digits_ram, max_digits_disk);
    ...
    std::cout << "Maximale Nachkommastellen (basierend auf RAM und Festplatte): " << max_allowed_digits << std::endl;
    ...
    } else if (desiredDecimalPlaces > static_cast<int>(max_allowed_digits)) {
        std::cerr << "Eingabe überschreitet das Maximum. Setze auf " << max_allowed_digits << " Nachkommastellen." << std::endl;
        desiredDecimalPlaces = static_cast<int>(max_allowed_digits);
    }
    // Aufgrund der double-Precision-Beschränkung (GPU) setzen wir ein sicheres Maximum von 700 Nachkommastellen
    const int safeMaxDigits = 700;
    if (desiredDecimalPlaces > safeMaxDigits) {
        std::cerr << "Warnung: Für double-Precision können maximal " << safeMaxDigits 
                  << " Nachkommastellen berechnet werden. Setze auf " << safeMaxDigits << "." << std::endl;
        desiredDecimalPlaces = safeMaxDigits;
    }
    
    // Für die GPU-Methode (hybrid) wird nur niedrige Präzision (bis ca. 16 Nachkommastellen) verwendet.
    // Für höhere Präzision wird die CPU-basierte multipräzise Berechnung genutzt.
    if (desiredDecimalPlaces <= 16) {
